...
export P1_LORA="${P1_LORA:-$OUT/train/run/epoch-2.safetensors}"
...
"$PYTHON_BIN" scripts/qwen_arch_control/infer_qwen_arch_incontext.py \
  ...
  --num-samples "${P1_INFER_SAMPLES:-20}" \
  --height "${P1_INFER_HEIGHT:-256}" \
  --width "${P1_INFER_WIDTH:-256}" \
  --num-inference-steps "${P1_INFER_STEPS:-20}" \
  --seed "${P1_SEED:-5521}" \
