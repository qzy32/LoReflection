...
mkdir -p "$OUT/eval" reports/prompt_label_generation reports/p1_small_metric_v2_goal_aligned_llm_summary
...
"$PYTHON_BIN" scripts/qwen_arch_control/evaluate_p0_sanity_outputs.py \
  ...
  --script scripts/qwen_arch_control/run_p1_small_metric_v2_goal_aligned_llm_train.sh \
  --script scripts/qwen_arch_control/run_p1_small_metric_v2_goal_aligned_llm_infer.sh \
  --script scripts/qwen_arch_control/infer_qwen_arch_incontext.py \
  --checkpoint-used "$P1_METRIC_V2_LORA" \
  --phase-label p1
cp "$OUT/eval/p0_sanity_eval_report.json" "$OUT/eval/goal_aligned_llm_eval_report.json"
"$PYTHON_BIN" scripts/qwen_arch_control/build_p0_infer_review_sheet.py \
  --output-root "$OUT" \
  --dataset-base "$DATASET_ROOT" \
  --metadata "$METADATA"
cp "$OUT/eval/p0_infer_review_sheet.png" "$OUT/eval/goal_aligned_llm_review_sheet.png"
cp "$OUT/eval/p0_infer_review.html" "$OUT/eval/goal_aligned_llm_review.html"
"$PYTHON_BIN" - <<'PY'
from pathlib import Path
import json, shutil
root=Path('/wuqingyaoa800/qiuziyan/LoReflection_arch_p0')
out=root/'outputs/qwen_arch_incontext_p1_small_metric_v2_goal_aligned_llm'
basic_path=root/'outputs/qwen_arch_incontext_p1_small_metric_v2/eval/p1_small_metric_v2_eval_report.json'
llm_path=out/'eval/goal_aligned_llm_eval_report.json'
...
llm.update({
  'experiment_name':'p1_small_metric_v2_goal_aligned_llm',
  'prompt_source':'qwen2_5_7b_goal_aligned_llm_variants',
  'metadata':'metadata_goal_aligned_relation_rich.csv',
...
for key in ['target_pixel_agreement_after_quantization','furniture_pixel_precision','furniture_pixel_recall','furniture_pixel_f1','palette_unknown_rate_after_quantization']:
    ...
    rows.append({'metric':key,'basic_metric_v2':b,'goal_aligned_llm':l,'delta':None if b is None or l is None else l-b})
comparison={'basic_report':str(basic_path),'goal_aligned_llm_report':str(llm_path),'metrics':rows}
...
(root/'reports/prompt_label_generation/goal_aligned_llm_training_comparison.json').write_text(json.dumps(comparison, ensure_ascii=False, indent=2)+'\n')
md=['# Goal-aligned LLM Training Comparison','','| metric | basic_metric_v2 | goal_aligned_llm | delta |','|---|---:|---:|---:|']
for r in rows:
    md.append(f"| {r['metric']} | {r['basic_metric_v2']} | {r['goal_aligned_llm']} | {r['delta']} |")
md += ['',f"decision: `{comparison['decision']}`",'', 'No P1-1000 was run.']
(root/'reports/prompt_label_generation/GOAL_ALIGNED_LLM_TRAINING_COMPARISON.md').write_text('\n'.join(md)+'\n')
summary=(root/'reports/p1_small_metric_v2_goal_aligned_llm_summary')
summary.mkdir(parents=True, exist_ok=True)
for src,name in [(llm_path,'goal_aligned_llm_eval_report.json'),(out/'eval/palette_quantization_report.json','palette_quantization_report.json'),(contract_path,'inference_command_contract_check.json')]:
    if src.exists(): shutil.copy2(src, summary/name)
summary_md=f"""# P1-small metric_v2 Goal-aligned LLM Training Summary

- experiment_name: `p1_small_metric_v2_goal_aligned_llm`
- prompt_source: `qwen2_5_7b_goal_aligned_llm_variants`
- dataset: `{root/'data/loreflection_qwen_arch_control_p1_small_metric_v2_goal_aligned_llm_prompts'}`
- metadata: `metadata_goal_aligned_relation_rich.csv`
...
"""
(summary/'GOAL_ALIGNED_LLM_TRAINING_SUMMARY.md').write_text(summary_md)
PY
