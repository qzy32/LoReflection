...
P1_NORM_LORA="$ROOT/outputs/qwen_arch_incontext_p1_small/train/run/epoch-2.safetensors"
P0_LORA="$ROOT/outputs/qwen_arch_incontext_p0_sanity/train_p0_50/run/epoch-2.safetensors"
BASE_UNION_LORA="$DIFFSYNTH_ROOT/models/DiffSynth-Studio/Qwen-Image-In-Context-Control-Union/model.safetensors"
if [ -z "${INIT_LORA:-}" ]; then
  if [ -f "$P1_NORM_LORA" ]; then INIT_LORA="$P1_NORM_LORA";
  elif [ -f "$P0_LORA" ]; then INIT_LORA="$P0_LORA";
  else INIT_LORA="$BASE_UNION_LORA"; fi
fi
export INIT_LORA
...
mkdir -p "$OUT/logs" "$OUT/train/run" "$OUT/eval" reports/p1_small_metric_v2_summary

if [ -z "${CUDA_VISIBLE_DEVICES:-}" ]; then
  SELECTED_GPU=$(nvidia-smi --query-gpu=index,memory.free --format=csv,noheader,nounits | awk -F, 'BEGIN{best=-1;idx=""}{gsub(/ /,"",$1); gsub(/ /,"",$2); if ($2>best){best=$2; idx=$1}} END{print idx}')
  export CUDA_VISIBLE_DEVICES="$SELECTED_GPU"
fi
echo "CUDA_VISIBLE_DEVICES=$CUDA_VISIBLE_DEVICES" | tee -a "$OUT/logs/gpu_selection.log"
